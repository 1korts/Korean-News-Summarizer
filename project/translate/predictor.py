...
def build_predictor(tokenizer, model):
    symbols = {'BOS': 2, 'EOS': 3,
               'PAD': 1, 'EOQ': 0}
    scorer = GNMTGlobalScorer(0.6, length_penalty='wu')
    return Translator(model, tokenizer, symbols, global_scorer=scorer)
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = pred.replace('[CLS]', '').replace('[PAD]', '').replace(
                        '[SEP]', '').replace(r' +', ' ').replace(' [UNK] ', '<q>').replace('[UNK]', '').strip()
                    gold_str = gold.strip()
